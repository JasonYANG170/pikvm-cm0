...
补齐 Janus 1.x 媒体轨道回调兼容后，Edge 网页已实际显示 **H.264 · 1024×768 · 60 fps**。
这是当前链路的观测值，不保证所有动态画面、网络条件或 1080p 下都达到同样帧率。
...
```bash
sudo /usr/bin/python3 scripts/update-web-touch.py --check
sudo /usr/bin/python3 scripts/update-web-touch.py
```

若已使用此前版本安装 WebRTC，可单独运行 `sudo /usr/bin/python3 scripts/fix-janus-web.py`，
修复旧网页的媒体回调以及页面关闭事件兼容问题。新版 WebRTC 安装脚本已包含此步骤。

仅修改旧版鼠标模块并添加独立 JS/CSS，不升级后端、不重启服务、不改密码。
脚本验证已知 `mouse.js` 校验和，未知版本拒绝覆盖，重复安装拒绝覆盖。
备份位于 `/var/backups/pikvm-web-*`；按脚本输出执行回滚后刷新浏览器即可。

- 单指定位、轻点左键单击；移动后抬手不会误触点击。
- 双指水平/垂直滚动；双指变单指时不会跳动或误点。
- 浮动左键、右键、拖拽锁定/松开和滚动按钮；触摸设备自动显示，也可点击标题栏“触控”切换。
- 手势取消、切换标签页、窗口失焦和连接断开会清理拖拽状态。
- 保留原有桌面鼠标、绝对/相对模式和后端 HID 消息格式。

本更新是功能移植，不宣称完整升级为最新版 PiKVM。验证：`node --test tests/touch.test.mjs`。
已通过 6 项输入回归测试、JavaScript 语法检查，并在实际网页验证触控面板显示和 H.264 播放。
手机/平板实机手势与被控电脑的 HID 响应仍需结合具体终端验证。
